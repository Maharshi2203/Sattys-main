...
export function Header() {
  ...
  // Pages where we want a dark text header by default
  const defaultDarkHeader = isCartPage
  ...
  return (
    <header className={cn(
      ...
      scrolled ? "bg-white/90 backdrop-blur-md shadow-sm py-3 sm:py-4" : "bg-transparent py-6 sm:py-8",
      defaultDarkHeader && !scrolled && "border-b border-black/5"
    )}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center gap-6 lg:gap-8 xl:gap-10">
              {navLinks.map((link) => (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    "relative text-base xl:text-lg font-medium tracking-wide transition-all cursor-pointer group",
                    scrolled || defaultDarkHeader ? "text-gray-900 hover:text-[#C8102B]" : "text-white hover:text-white/80"
                  )}
                >
                  {link.label}
                  <span className={cn(
                    "absolute -bottom-1 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full",
                    scrolled || defaultDarkHeader ? "bg-[#C8102B]" : "bg-white"
                  )} />
                </Link>
              ))}
            </nav>
          ...
          <button
            className={cn(
              "lg:hidden p-1.5 sm:p-2 transition-all flex-shrink-0",
              scrolled || defaultDarkHeader ? "text-gray-900" : "text-white"
            )}
        ...
        {/* Mobile Menu */}
        <div className={cn(
          "lg:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl transition-all duration-300 overflow-hidden z-[100] shadow-xl",
